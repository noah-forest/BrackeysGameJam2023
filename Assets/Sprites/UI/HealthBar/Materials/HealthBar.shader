Shader "CustomShaders/HealthBar"
{
	Properties
	{
		...
		_wavePixelCount("Fill Wave Pixel Count", float) = 5
		...
		SubShader
		{
			...
			float _waveAmp, _waveFreq, _waveSpeed, _wavePixelCount;
			CBUFFER_END
			...
			float4 frag(Varyings IN) : SV_Target
			{
				...
				float waveOffset = _waveAmp * floor(cos(_waveFreq * (IN.uv.x + _Time.y * _waveSpeed)) * _wavePixelCount) * floor(min(1.3f * sin(PI * _healthNormalized), 1) * _wavePixelCount);
